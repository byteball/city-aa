...
describe('City', function () {
	...
	it('Deploy City AA', async () => {
		...
		city = city.replace(/\$replicator = '\w*'/, `$replicator = '${this.founderAddress}'`)
		city = city.replace(/\$old_city_aa = '\w*'/, `$old_city_aa = '${this.network.agent.old_city}'`)
	...
	it('Initialize the old city', async () => {
		...
		const { unit, error } = await this.founder.triggerAaWithData({
			toAddress: this.network.agent.old_city,
			amount: 10000,
			data: {
	...
	it('Replicator copies the old city vars (only one var in this test)', async () => {
		const { unit, error } = await this.founder.triggerAaWithData({
			toAddress: this.city_aa,
			amount: 10000,
			data: {
	...
	it('Replicator distributes the initial CITY', async () => {
		const outputs = [
			{ address: this.aliceAddress, amount: 100e12 },
			{ address: this.bobAddress, amount: 100e12 },
		];
		const { unit, error } = await this.founder.triggerAaWithData({
			toAddress: this.city_aa,
			amount: 10000,
		...
		expect(Utils.getExternalPayments(unitObj)).to.deep.equalInAnyOrder(outputs.map(o => ({ ...o, asset: this.asset })))
	})


	it('Replicator renounces his rights', async () => {
		const { unit, error } = await this.founder.triggerAaWithData({
			toAddress: this.city_aa,
			amount: 10000,
			data: {
		...
		const { vars } = await this.founder.readAAStateVars(this.city_aa)
		expect(vars.constants).to.deep.eq({
			asset: this.asset,
			governance_aa: this.governance_aa,
			renounced: true,
		})
	})

	it('Replicator tries to copy the old city vars after renouncing', async () => {
		const { unit, error } = await this.founder.triggerAaWithData({
			toAddress: this.city_aa,
			amount: 10000,
			data: {
	...
	it('Alice tries to buy land while not being attested', async () => {
		...
		expect(response.response.error.message).to.eq("your address must be attested")
	})
	...
	it('Alice buys land', async () => {
		...
		const { response } = await this.network.getAaResponseToUnitOnNode(this.alice, unit)
		console.log(response.response.error)
		expect(response.response.error?.message).to.be.undefined
		expect(response.bounced).to.be.false
		expect(response.response_unit).to.be.null
	...
	it('Bob buys land until he becomes a neighbor with Alice', async () => {
		...
		while (true) {
			...
			const { unit: buy_unit, error: buy_error } = await this.bob.sendMulti({
				outputs_by_asset: {
					[this.asset]: [{ address: this.city_aa, amount: amount }],
					base: [{ address: this.city_aa, amount: 10000 }],
				},
		...
		//	console.log({ buy_error, unit: buy_unit })
			...
			expect(buy_response.response_unit).to.be.null
			expect(buy_response.response.responseVars.plot_num).to.eq(plot_num)

			const timestamp = buy_response.timestamp
	...
	it("Launch the CITY token and try to buy with Bytes", async () => {
		...
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.validUnit
		expect(response.response.error.message).to.eq("neither case is true in messages")
	...
	it("Bob tries to transfer a plot to the VRF oracle, who is not attested", async () => {
		...
		const { response } = await this.network.getAaResponseToUnitOnNode(this.bob, unit)
		console.log(response.response.error)
		expect(response.response.error.message).to.be.eq("new owner's address must be attested")
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
	})
	...
	it("Bob edits his house and tries to use an invalid shortcode", async () => {
		...
		const { response } = await this.network.getAaResponseToUnitOnNode(this.bob, unit)
		console.log(response.response.error)
		expect(response.response.error.message).to.eq("shortcode is allowed to include only lowercase latin letters, numbers, -, _, and .")
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
	})
	...
	it("Alice edits her house and tries to use bob's shortcode", async () => {
		...
		const { response } = await this.network.getAaResponseToUnitOnNode(this.alice, unit)
		console.log(response.response.error)
		expect(response.response.error.message).to.eq("this shortcode is already taken")
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
	...
	it("Claim follow-up reward", async () => {
		...
		expect(alice_response.response.error?.message).to.be.undefined
		expect(alice_response.bounced).to.be.false
		expect(alice_response.response_unit).to.be.null
		expect(alice_response.response.responseVars.message).to.eq("Registered your request. Your neighbor must send their request within 10 minutes, otherwise you both will have to start over.")
		...
		const { response: bob_response } = await this.network.getAaResponseToUnitOnNode(this.bob, bob_unit)
		expect(bob_response.response.error?.message).to.be.undefined
		expect(bob_response.bounced).to.be.false
		expect(bob_response.response_unit).to.be.null
